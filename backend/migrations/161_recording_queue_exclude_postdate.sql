-- ============================================================================
-- 161_recording_queue_exclude_postdate.sql
-- The recording-review queue must surface SALES ONLY — not un-charged post-dated
-- sales. A post-date sale lives only in its "Post Date" tab until "Charge → Sale"
-- flips closer_disposition to 'sale'; until then it isn't a reviewable sale, so it
-- shouldn't appear in the recording-review queue (and thus shouldn't flow to the
-- client portal). This mirrors what compliance's "All Sales" + Queue already do
-- (routes/compliance.js exclude_post_date): hide rows whose closer_disposition
-- ILIKE '%post%date%'. NULL-safe. Charged post-dates (disposition flipped to
-- 'sale') keep showing.
--
-- ONLY change vs 160: one added WHERE clause. Everything else identical.
-- CREATE OR REPLACE — safe to re-run; supersedes 160. Apply AFTER 150-160.
-- ============================================================================
...
BEGIN
  ...
  RETURN QUERY EXECUTE format($q$
    ...
    WHERE (($5 = 'all') OR ($5 = 'pending' AND COALESCE(cc.n,0) = 0) OR ($5 = 'confirmed' AND COALESCE(cc.n,0) > 0))
      -- SALES ONLY: hide un-charged post-dated sales (charged ones flip to 'sale').
