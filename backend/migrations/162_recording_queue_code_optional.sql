-- ============================================================================
-- 162_recording_queue_code_optional.sql
-- The recording-review queue was requiring the transfer to carry a
-- vicidial_vendor_code (mig 160 = "any code", but still NOT NULL). That hid
-- legit sales whose transfer has no code — even though a MAPPED closer's
-- recording resolves fine via the agent+date+phone path (no code needed). The
-- real eligibility is "the closer is mapped to a dialer agent id"; the code is
-- optional context, not a gate.
--
-- Changes vs 161:
--   * transfers join: INNER + "vendor_lead_code IS NOT NULL"  →  LEFT JOIN
--     (a sale with no transfer / no code still lists if the closer is mapped).
--   * user_profiles mapped-closer requirement is UNCHANGED (still required —
--     an unmapped closer's recording can't be resolved at all).
--   * post-date exclusion (161) and everything else identical.
-- CREATE OR REPLACE — safe to re-run; supersedes 161. Apply AFTER 150-161.
-- ============================================================================
...
BEGIN
  ...
  RETURN QUERY EXECUTE format($q$
    ...
    FROM sales s
    LEFT JOIN transfers t ON t.id = s.transfer_id            -- code now optional
