...
const { getBatchRules, isDialerRecipient, ruleExclusions } = require('../utils/batchRules');
...
// Run async work in concurrent batches (fast, but bounded so we don't flood PG).
const runParallel = async (items, fn, conc = 50) => {
  for (let i = 0; i < items.length; i += conc) await Promise.all(items.slice(i, i + conc).map(fn));
};
api.post('/backfill/from-list', superOnly, asyncHandler(async (req, res) => {
  ...
  // chunked .in() reader (PostgREST caps URL length)
  const fetchIn = async (table, col, sel, vals) => {
    const out = [];
    for (let i = 0; i < vals.length; i += 200) {
      const { data } = await supabaseAdmin.from(table).select(sel).in(col, vals.slice(i, i + 200));
  ...
  for (const g of groups) {
    ...
    if (ph && leads.length) {
      ...
      phoneLeads.get(ph).push(...leads);
    }
  }
  const phones = [...phoneLeads.keys()];
  let matched = 0, applied = 0, skippedStatus = 0, noMatch = 0;
  if (!phones.length) return res.json({ ok: true, batch_id: batchId, received, matched, applied, skipped_status: 0, no_match: received });

  // ── 2. ALL code-less transfers for these phones (one bulk read) ─────────────
  const allTrs = [];
  for (let i = 0; i < phones.length; i += 200) {
    const { data } = await supabaseAdmin.from('transfers')
      .select('id, company_id, created_at, vicidial_dispo, assigned_closer_id, normalized_phone')
      .in('normalized_phone', phones.slice(i, i + 200)).is('vicidial_vendor_code', null)
      .order('created_at', { ascending: true });
    if (data) allTrs.push(...data);
  }
  const trsByPhone = new Map();
  for (const t of allTrs) { if (!trsByPhone.has(t.normalized_phone)) trsByPhone.set(t.normalized_phone, []); trsByPhone.get(t.normalized_phone).push(t); }

  // ── 3. which of those already carry a disposition (no-overwrite) ────────────
  const disposed = new Set();
  (await fetchIn('disposition_actions', 'transfer_id', 'transfer_id', allTrs.map(t => t.id))).forEach(a => disposed.add(a.transfer_id));

  // ── 4. plan pairings in memory — nearest-date first, NO hard window so every
  //       transfer that has any lead on its phone gets one (coverage first) ─────
  const plan = [];
  for (const [ph, leads] of phoneLeads) {
    const trs = trsByPhone.get(ph) || [];
    const distinct = new Set(leads.map(l => l.leadId)).size;
    if (!trs.length) { noMatch += distinct; continue; }
    const pairs = [];
    for (let ti = 0; ti < trs.length; ti++) {
      const trT = Date.parse(trs[ti].created_at);
      for (let li = 0; li < leads.length; li++) {
        const lt = leads[li].t;
        pairs.push({ ti, li, diff: Number.isFinite(lt) ? Math.abs(lt - trT) : Number.MAX_SAFE_INTEGER });
      }
    }
    pairs.sort((a, b) => a.diff - b.diff);
    const usedT = new Set(), usedLid = new Set();
    for (const p of pairs) {
      const lid = leads[p.li].leadId;
      if (usedT.has(p.ti) || usedLid.has(lid)) continue;
      usedT.add(p.ti); usedLid.add(lid);
      const tr = trs[p.ti], lead = leads[p.li];
      matched++;
      const real = !LIST_SKIP.has(lead.status);
      const already = !!tr.vicidial_dispo || disposed.has(tr.id);
      plan.push({ tr, status: lead.status, code: prefix + lead.leadId, setDispo: real && !already });
    }
    noMatch += distinct - usedLid.size;
  }

  if (dryRun) return res.json({ ok: true, batch_id: batchId, received, matched, applied: 0, skipped_status: 0, no_match: noMatch });

  // ── 5. drop collisions (code already on another transfer) + intra-batch dups ─
  const existing = new Set();
  (await fetchIn('transfers', 'vicidial_vendor_code', 'vicidial_vendor_code', [...new Set(plan.map(p => p.code))]))
    .forEach(e => existing.add(e.vicidial_vendor_code));
  const seen = new Set();
  const finalPlan = plan.filter(p => { if (existing.has(p.code) || seen.has(p.code)) return false; seen.add(p.code); return true; });

  // ── 6. dispo name + config maps, in memory ──────────────────────────────────
  const { data: dmapRows } = await supabaseAdmin.from('vicidial_dispo_map').select('company_id, vici_code, disposition_name');
  const dmap = new Map();
  (dmapRows || []).forEach(m => { if (m.disposition_name) dmap.set(`${m.company_id || ''}|${m.vici_code}`, m.disposition_name); });
  const nameFor = (co, code) => dmap.get(`${co}|${code}`) || dmap.get(`|${code}`) || code;
  const { data: cfgRows } = await supabaseAdmin.from('disposition_configs').select('id, name, color, company_id').eq('is_active', true);
  const cfgMap = new Map();
  (cfgRows || []).forEach(c => cfgMap.set(`${c.company_id || ''}|${c.name}`, c));
  const cfgFor = (co, name) => cfgMap.get(`${co}|${name}`) || cfgMap.get(`|${name}`) || null;

  // ── 7. apply transfer updates — ONE bulk RPC (migration 114), with a
  //       per-row parallel fallback if the function isn't there yet ────────────
  const items = finalPlan.map(p => ({ id: p.tr.id, code: p.code, dispo: p.setDispo ? p.status : null }));
  let usedRpc = false;
  if (items.length) {
    try {
      for (let i = 0; i < items.length; i += 2000) {
        const { error } = await supabaseAdmin.rpc('backfill_apply_transfers', { items: items.slice(i, i + 2000) });
  ...
  if (!usedRpc) {
    await runParallel(finalPlan, async (p) => {
      const upd = { vicidial_vendor_code: p.code };
      if (p.setDispo) { upd.vicidial_dispo = p.status; upd.vicidial_dispo_at = now; }
      await supabaseAdmin.from('transfers').update(upd).eq('id', p.tr.id);
    });
  }

  // ── 8. ONE bulk insert of the visible disposition rows ──────────────────────
  const dispoPlan = finalPlan.filter(p => p.setDispo);
  if (dispoPlan.length) {
    const rows = dispoPlan.map(p => {
      const name = nameFor(p.tr.company_id, p.status);
      const cfg = cfgFor(p.tr.company_id, name);
      return {
        transfer_id: p.tr.id, company_id: p.tr.company_id, user_id: p.tr.assigned_closer_id || req.user.id,
        disposition_config_id: cfg?.id || null, disposition_name: name, color: cfg?.color || '#6b7280',
        note: `Backfill from list (${p.status})`, setter_role: 'closer',
      };
    });
    for (let i = 0; i < rows.length; i += 500) { try { await supabaseAdmin.from('disposition_actions').insert(rows.slice(i, i + 500)); } catch { /* non-critical */ } }
  }

  applied = finalPlan.length;
  skippedStatus = finalPlan.filter(p => !p.setDispo).length;

  // ── 9. bulk insert fills + roll batch totals (undo removes actions by note) ──
  if (batchOk) {
    const fillRows = finalPlan.map(p => ({
      batch_id: batchId, transfer_id: p.tr.id, prev_dispo: p.tr.vicidial_dispo || null,
      new_dispo: p.setDispo ? p.status : null, new_code: p.code, dispo_action_id: null,
    }));
    for (let i = 0; i < fillRows.length; i += 500) { try { await supabaseAdmin.from('vicidial_backfill_fills').insert(fillRows.slice(i, i + 500)); } catch { /* non-critical */ } }
    try {
      const { data: b } = await supabaseAdmin.from('vicidial_backfill_batches').select('total_rows, applied_count').eq('id', batchId).maybeSingle();
      await supabaseAdmin.from('vicidial_backfill_batches')
        .update({ total_rows: (b?.total_rows || 0) + received, applied_count: (b?.applied_count || 0) + applied }).eq('id', batchId);
    } catch { /* non-critical */ }
  }
...
api.post('/backfill/batches/:id/undo', superOnly, asyncHandler(async (req, res) => {
  ...
  // Bulk-read current state of every filled transfer, then revert in parallel.
  const ids = [...new Set((fills || []).map(f => f.transfer_id))];
  const cur = new Map();
  for (let i = 0; i < ids.length; i += 200) {
    const { data } = await supabaseAdmin.from('transfers')
      .select('id, vicidial_dispo, vicidial_vendor_code').in('id', ids.slice(i, i + 200));
    (data || []).forEach(t => cur.set(t.id, t));
  }
  let undone = 0, skipped = 0;
  await runParallel((fills || []), async (f) => {
    const tr = cur.get(f.transfer_id);
    ...
    if (!Object.keys(updates).length) { skipped++; return; }  // changed since → leave it
    await supabaseAdmin.from('transfers').update(updates).eq('id', f.transfer_id);
    // Remove the visible backfill row: by id if we recorded it (legacy), else by
    // its exact note on that transfer.
    if (f.dispo_action_id) await supabaseAdmin.from('disposition_actions').delete().eq('id', f.dispo_action_id);
    else if (f.new_dispo) await supabaseAdmin.from('disposition_actions').delete().eq('transfer_id', f.transfer_id).eq('note', `Backfill from list (${f.new_dispo})`);
    undone++;
  });
  await supabaseAdmin.from('vicidial_backfill_batches')
...
// ════════════════════════════════════════════════════════════════════════════
// Agent Numbers puller (superadmin) — STATELESS proxy over VICIdial's admin
// user_stats.php. Fetch (Basic-auth creds injected from app_secrets, never sent
// to the browser), parse the HTML report → JSON, return it. The CRM stores NONE
// of the pulled data — it lives only in the caller's browser. The ONLY thing
// persisted is a batch the superadmin deliberately builds (/stats/create-batch),
// via the existing distribution_batches flow → each recipient's "My Numbers".
// ════════════════════════════════════════════════════════════════════════════

// Boxes + which have user_stats creds set (never returns the secret itself).
api.get('/stats/config', superOnly, asyncHandler(async (req, res) => {
  res.json({ boxes: await vstats.authStatus() });
}));

// Set (write-only) a box's user_stats Basic-auth creds → app_secrets.
api.post('/stats/auth', superOnly, asyncHandler(async (req, res) => {
  const boxId = String(req.body.box_id || '').trim();
  const user  = String(req.body.user || '').trim();
  const pass  = String(req.body.pass ?? '');
  if (!boxId || !user) return res.status(400).json({ error: 'box_id and user are required' });
  if (!vstats.boxById(boxId)) return res.status(404).json({ error: 'Unknown box' });
  await vstats.setStatsAuth(boxId, user, pass, req.user.id);
  logger.success('VICI_STATS', `user_stats creds set for box ${boxId} by ${req.user.id}`);
  res.json({ ok: true });
}));

// Pull one or several agents' user_stats reports for a date window. Returns
// parsed JSON per agent; the CRM persists NONE of it.
api.post('/stats/pull', superOnly, asyncHandler(async (req, res) => {
  ...
  const endDate   = String(req.body.end_date || '').slice(0, 10) || beginDate;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(beginDate)) return res.status(400).json({ error: 'begin_date (YYYY-MM-DD) is required' });

  const boxId      = String(req.body.box_id || '').trim() || null;
  const callStatus = String(req.body.call_status || '').trim();
  const archived   = !!req.body.archived;
  const db         = req.body.db != null ? (parseInt(req.body.db, 10) || 0) : 0;
  ...
  results.sort((a, b) => agents.indexOf(a.agent) - agents.indexOf(b.agent));   // stable request order
  res.json({ ok: true, begin_date: beginDate, end_date: endDate, agents: results });
}));

// Resolve the final recipient user-id list for a create-batch request.
async function resolveStatsRecipients(body) {
  const target = body.target || (body.recipient_id ? 'user' : 'users');
  if (target === 'user' && body.recipient_id) return [body.recipient_id];
  if (target === 'users') return [...new Set((body.recipient_ids || []).filter(Boolean))];
  if (target === 'team' && body.team_id) {
    const { data } = await supabaseAdmin.from('team_members').select('user_id').eq('team_id', body.team_id);
    return [...new Set((data || []).map(r => r.user_id).filter(Boolean))];
  }
  if (target === 'company' && body.company_id) {
    const { data } = await supabaseAdmin.from('user_company_roles')
      .select('user_id').eq('company_id', body.company_id).eq('is_active', true);
    return [...new Set((data || []).map(r => r.user_id).filter(Boolean))];
  }
  return [];
}

// Even split of `items` into n buckets — sequential contiguous slices, or a
// Fisher-Yates shuffle first for random dealing. Remainder → the first buckets.
function splitStatsNumbers(items, n, mode) {
  const pool = [...items];
  if (mode === 'random') for (let x = pool.length - 1; x > 0; x--) { const j = Math.floor(Math.random() * (x + 1)); [pool[x], pool[j]] = [pool[j], pool[x]]; }
  const base = Math.floor(pool.length / n), rem = pool.length % n;
  const out = []; let idx = 0;
  for (let k = 0; k < n; k++) { const take = base + (k < rem ? 1 : 0); out.push(pool.slice(idx, idx + take)); idx += take; }
  return out;
}

// Create distribution batch(es) from the pulled numbers → hand to users. Reuses
// the distribution_batches tables + rule-exclusion filter, so the numbers land
// in each recipient's floating "My Numbers" widget. This is the ONLY persistence.
// distribute: 'split' (divide numbers across recipients) | 'copy' (each gets all).
api.post('/stats/create-batch', superOnly, asyncHandler(async (req, res) => {
  const name       = String(req.body.name || 'Agent numbers').slice(0, 200);
  const distribute = req.body.distribute === 'copy' ? 'copy' : 'split';
  const mode       = req.body.mode === 'random' ? 'random' : 'sequential';

  // Sanitize incoming numbers → distinct last-10 phones (with optional name/lead).
  const seen = new Set();
  const items = [];
  for (const n of (Array.isArray(req.body.numbers) ? req.body.numbers : [])) {
    const d = String(n?.phone || '').replace(/\D/g, '');
    ...
    items.push({
      phone_number: d,
      customer_name: (n.customer_name ? String(n.customer_name).slice(0, 200) : null),
      lead_id: (String(n.lead_id || '').replace(/\D/g, '') || null),
    });
  }
  if (!items.length) return res.status(400).json({ error: 'No valid phone numbers to send' });

  const recipients = await resolveStatsRecipients(req.body);
  if (!recipients.length) return res.status(400).json({ error: 'No recipients — pick a user, users, team, or company' });
  if (distribute === 'split' && recipients.length > items.length) {
    return res.status(400).json({ error: 'More recipients than numbers — use "copy to each" or add numbers' });
  }

  const plan = distribute === 'copy'
    ? recipients.map(rid => ({ recipient_id: rid, items }))
    : splitStatsNumbers(items, recipients.length, mode).map((chunk, k) => ({ recipient_id: recipients[k], items: chunk }));

  const created = [];
  const summary = [];
  try {
    for (const part of plan) {
      if (!part.items.length) continue;   // a 0-count bucket gets no batch
      const { data: rcr } = await supabaseAdmin.from('user_company_roles')
        .select('company_id').eq('user_id', part.recipient_id).eq('is_active', true)
        .order('created_at', { ascending: true }).limit(1).maybeSingle();
      const companyId = rcr?.company_id || null;

      const { data: batch, error: bErr } = await supabaseAdmin.from('distribution_batches').insert({
        name, created_by: req.user.id, parent_batch_id: null, source: 'vici_user_stats',
        sent_to_user_id: part.recipient_id, company_id: companyId, item_count: part.items.length,
      }).select().single();
      if (bErr) throw new Error(bErr.message);
      created.push(batch.id);

      // Rule filter only bites at the final hop (a dialer/fronter recipient).
      const rules  = await getBatchRules(companyId);
      const dialer = await isDialerRecipient(part.recipient_id);
      const exMap  = dialer ? await ruleExclusions(part.items.map(i => i.phone_number), part.recipient_id, companyId, rules) : new Map();

      const rows = part.items.map((it, idx) => {
        const reason = exMap.get(it.phone_number);
        return { batch_id: batch.id, position: idx + 1, phone_number: it.phone_number, customer_name: it.customer_name, lead_id: it.lead_id, ...(reason ? { status: 'excluded', exclusion_reason: reason } : {}) };
      });
      for (let j = 0; j < rows.length; j += 1000) {
        const { error: iErr } = await supabaseAdmin.from('distribution_batch_items').insert(rows.slice(j, j + 1000));
        if (iErr) throw new Error(iErr.message);
      }

      notifications.notifyUsers([part.recipient_id], {
        type: 'batch_received', title: 'New batch received',
        message: `${req.user.name || 'A superadmin'} sent you "${name}" (${part.items.length - exMap.size} numbers).`,
        companyId, data: { batch_id: batch.id, kind: 'distribution_batch' }, dedupBase: `batch_${batch.id}`,
      }).catch(() => {});
      summary.push({ batch_id: batch.id, recipient_id: part.recipient_id, item_count: part.items.length, excluded_count: exMap.size });
    }
  } catch (e) {
