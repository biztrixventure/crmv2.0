    ...
    const load = async () => {
      try {
        const res = await client.get('callbacks', {
          params: { company_id: companyId, user_id: userId, limit: 1000, page: 1 },
        });
      ...
      <ModalBox>
        <div className="flex items-start justify-between p-5" style={{ borderBottom: '1px solid var(--color-border)' }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'var(--gradient-sidebar)' }}>
              <User size={18} className="text-white" />
            </div>
            <div>
              <h3 className="font-bold text-text">{userName}</h3>
              <p className="text-xs text-text-secondary">Agent callback report</p>
            </div>
          </div>
  ...
  return (
    <div>
      <TabHeader
        ...
        subtitle={`Callbacks for ${user?.company_name || 'your company'}`}
            ...
            <div className="overflow-y-auto p-6 space-y-5">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <InfoTile label="Status" value={
                    <Badge variant={STATUS_BADGE[detail.status] || 'secondary'} size="sm">
                  ...
                  <InfoTile label="Agent" value={
                    detail.user_name ? (
                      <button onClick={() => { setDetail(null); setAgentStats({ userId: detail.user_id, userName: detail.user_name }); }}
...
export default ManagerCallbacksTab;
